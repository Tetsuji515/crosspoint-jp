...
// Same "clock is plausible" threshold as SleepActivity::isTimeValid():
// 2024-01-01 00:00:00 UTC. Unsynced ESP time sits near the 1970 epoch.
// TZ is set to JST-9 at boot (main.cpp), so localtime_r() yields JST.
...
Rect LauncherActivity::computeListRect() const {
  ...
  // The launcher runs portrait-only (supportsLandscape() is false), so the
  // button-hint strip occupies the bottom edge -- or the top edge in inverted
  // portrait (BaseTheme::drawButtonHints places it there).
  const int hintH = UITheme::getInstance().getMetrics().buttonHintsHeight;
  const bool hintsAtTop = renderer.getOrientation() == GfxRenderer::Orientation::PortraitInverted;

  const int listTop = HEADER_HEIGHT + (hintsAtTop ? hintH : 0);
  const int listBottom = pageHeight - (hintsAtTop ? 0 : hintH);
  return Rect{0, listTop, pageWidth, std::max(0, listBottom - listTop)};
}
...
void LauncherActivity::renderHeader() const {
  ...
  if (synced) {
    ...
    localtime_r(&now, &tmNow);  // TZ=JST-9, set at boot
...
void LauncherActivity::render(RenderLock&&) {
  switch (pendingScope) {
    ...
    case RenderScope::ListOnly: {
      // This base has no windowed-update wrapper (the old base's
      // setPartialUpdateRect), so redraw just the list region in the buffer
